export const particles = () => {
  ...
  const randomColor = () =>
    pick(["white", "cyan", "orange", "deepskyblue", "deeppink", "yellow"]);

  const bugs = Array.from({ length: 900 }, () => ({
    color: randomColor(),
    x: (random() < 0.5 ? 0 : 1) * (innerWidth - 1),
    y: (random() < 0.5 ? 0 : 1) * (innerHeight - 1),
    angle: random() * 2 * PI,
    phi: 1 + random() * 5,
  }));
  //这里有学到
  ...
  function move(bug) {
    ...
    bug.x += xr;
    bug.y += yr;
    bug.angle += 0.03 * Math.sin(performance.now() * 1e-3 * bug.phi);
